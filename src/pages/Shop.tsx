      ...
      <div className="max-w-[1800px] mx-auto px-4 md:px-8 lg:px-12">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between pb-8 mb-8 border-b border-white/10 gap-6">
          <div>
            ...
            <h1 className="text-3xl sm:text-5xl md:text-6xl font-light font-display tracking-[0.15em] uppercase text-white">
              SHOP ALL
            </h1>
          </div>

          <div className="text-xs font-mono text-[#8A8A8A] tracking-widest uppercase">
            SHOWING [{filteredProducts.length} OF {products.length} GARMENTS]
          </div>
        </div>

        {/* Filter & Sorting Controls Bar */}
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pb-6 mb-8 border-b border-white/5">
          {/* Category Tabs */}
            ...
            <button
              ...
              className={`px-3 py-1.5 uppercase tracking-widest border transition-colors whitespace-nowrap ${
                selectedCategory === 'ALL'
                  ...
                  : 'text-[#8A8A8A] border-white/10 hover:border-white/30'
              }`}
            >
              ALL [ {products.length} ]
            </button>
          ...
          <div className="flex items-center space-x-4 text-xs font-mono">
            ...
            <div className="hidden sm:flex items-center space-x-2">
              <span className="text-[#8A8A8A] uppercase">SIZE:</span>
            ...
            <div className="flex items-center space-x-2 ml-auto sm:ml-0">
              <span className="text-[#8A8A8A] uppercase">SORT:</span>
              <select
                value={sortBy}
                onChange={(e) => setSortBy(e.target.value as SortOption)}
                className="bg-[#111] border border-white/15 px-3 py-1.5 text-xs font-mono text-[#F4F4F0] focus:outline-none focus:border-white uppercase"
              >
        ...
        {filteredProducts.length === 0 ? (
          <div className="py-24 text-center border border-dashed border-white/10 p-8">
            <h3 className="text-xl font-light tracking-widest uppercase text-white mb-2">
            ...
            <button
              ...
              className="px-6 py-3 border border-white/20 hover:border-white text-xs font-mono tracking-widest uppercase text-white transition-colors"
            >
              RESET ALL FILTERS
            </button>
          </div>
